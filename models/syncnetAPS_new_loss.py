...
class DownBlock1d(nn.Module):
    ...
    def __init__(self, in_features, out_features, kernel_size, padding, stride = 1, circular_pad = False, apspool_criterion = 'l2', N=None):
        super(DownBlock1d, self).__init__()
        self.conv = nn.Conv1d(
            in_channels=in_features,
            out_channels=out_features,
...
class SyncNet(nn.Module):
    ...
    def forward(self, image, audio):
        face_embedding = self.face_encoder(image)
        audio_embedding = (
            self.audio_encoder(audio)
            .unsqueeze(2)
            .unsqueeze(3)
            .repeat(1, 1, face_embedding.size(2), face_embedding.size(3))
        )
        # print("image_embedding",image_embedding.shape) # [24, 128, 8, 8]
        # print("audio_embedding",audio_embedding.shape) # [24, 128, 8, 8]
